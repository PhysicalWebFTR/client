...
export default class Home extends Component {
  ...
  componentWillMount() {
    ...
    if(Platform.OS === 'android' && Platform.Version >= 23){
        PermissionsAndroid.check(PermissionsAndroid.PERMISSIONS.ACCESS_COARSE_LOCATION).then((result) => {
          console.log('ni pertama', result)
          if(!result){
            PermissionsAndroid.request(PermissionsAndroid.PERMISSIONS.ACCESS_COARSE_LOCATION).then((result) => {
              console.log('kedua', result)
              if(!result){
  ...
  componentDidMount() {
    ...
    bleManagerEmitter.addListener('BleManagerDiscoverPeripheral', (peripheral) => {
      console.log('state discover peripheral : ', peripheral)
    ...
    bleManagerEmitter.addListener(
      'BleManagerStopScan',
      () => {
        console.log('state bleManagerStopScan')
  ...
  connect = (peripheral_id) => {
    console.log('state connect: ', peripheral_id)
    // BleManager.connect(peripheral_id)
    BleManager.connect('B8:27:EB:5F:7C:84')
      .then(() => {
        this.setState({
        ...
        BleManager.retrieveServices('B8:27:EB:5F:7C:84')
          .then((peripheralInfo) => {
            console.log('Peripheral info:', peripheralInfo);
            BleManager.disconnect('B8:27:EB:5F:7C:84')
              .then(() => {
  ...
  attend = (value) => {
    ...
    console.log('state attend : ', value)
    this.setState({
      user_id: user_id
    });
    ...
    console.log('connected peripheral : ' , this.state.connected_peripheral)
